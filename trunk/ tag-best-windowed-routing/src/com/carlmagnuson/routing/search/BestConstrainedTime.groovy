...
class BestConstrainedTime {
	
	private static Logger log = Logger.getInstance(BestConstrainedTime.class)
	
	public static Route findRoute(Graph g, Node s, Node d, int earliestLeave, int latestArrival){
...
//					for(int t = g.getMinTime(); t<=g.getMaxTime(); t++){
		...
		for(; time<=g.getMaxTime(); time++){
			log.trace("Time: "+time+" Cost: "+timeSeries.get(time));
			if(timeSeries.get(time)<leastCost){
